            ...
            success: function (reqData) {
                ...
                if (news.length > 0) {
                    ...
                    $.each(news.slice(0,6), function (index, item) {
                        str += "<tr>" + "<th scope='row'>" + (index+1) + "</th>"+ "<td>" + item.newsid + "</td>" + "<td>" + item.newstitle + "</td>"+ "<td>" + item.newscontent + "</td>" + "<td>" + item.newsfrom + "</td>" + "<td>" + item.newstime + "</td>"
...
function update(news) {
    ...
    $("#typeName").val(news.newstitle);
    $("#typeprice").val(news.newscontent);
    $("#typesales").val(news.newsfrom);

    sessionStorage.setItem("upnews",JSON.stringify(news));
}
